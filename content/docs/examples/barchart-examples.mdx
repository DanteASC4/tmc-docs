...
Just a reminder that `barchart` returns an SVG, so to use the output you'll want to add it to the DOM however your framework of choice handles it.
...
document.body.setHTML(myChart);
```

<StaticChart options={{ data: [50, 100, 30] }} type="barchart" />
...
<StaticChart
  options={{ data: [50, 1000, 100], placement: 'left' }}
  type="barchart"
/>
...
<StaticChart
  options={{ data: [50, 1000, 100], placement: 'top' }}
  type="barchart"
/>
...
Because JGMC outputs SVG strings that you can instantly turn into a valid SVG element, they can be animated! The <u className="text-purple-500 font-bold">caveat</u> is that to animate the individual bars, depending on what sort of animation you want to do, you'll need to change the `transform-origin` for bars after the charts created based on which side they're on. Which sounds more complicated than it is!
...
<iframe
  height="400"
  width="400"
  src="/jgmc/example-pages/barchart-animejs/index.html"></iframe>
...
<LiveChart initialCode={`
import { barchart } from "@jgmc/vanilla";

const svgString = barchart({ data: [10, 50, 30] });
