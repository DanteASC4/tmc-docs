...
Just Give Me Charts (JGMC) is a lightweight, work-in-progress, clientside & soon to be ssr-friendly library built with deno to rapidly create inline SVG charts with minimal overhead.

<Callout title="Want a more in-depth explanation?">
  Head over to the [Why JGMC](/docs/why-jgmc) page to learn more about the
  motivation & purpose of JGMC.
</Callout>
...
```bash tab="pnpm"
pnpm install @jgmc/vanilla
```

```bash tab="yarn"
yarn add @jgmc/vanilla
```

```bash tab="bun"
bun install @jgmc/vanilla
```

```bash tab="deno"
deno install --npm @jgmc/vanilla
```
